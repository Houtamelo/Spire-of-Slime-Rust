...
use gdnative::api::{Directory, Engine, File, Node, OS, ProjectSettings};
...
#[extends(Node)]
pub struct SavesSingleton {
	saves: HashMap<String, SaveFile>,
}

#[methods]
impl SavesSingleton {
	pub fn godot_singleton() -> TInstance<'static, SavesSingleton, Shared> {
		...
		return singleton_node.cast_instance::<SavesSingleton>().expect("Failed to cast singleton SavesManager to SavesManager");
	}
	...
	fn _ready(&mut self, #[base] _owner: &Node) {
		...
		if save_folder.open(save_folder_path).is_err() {
			if let Err(error) = save_folder.make_dir_recursive(save_folder_path) {
				godot_error!("Failed to create save folder: {}", error);
			}
