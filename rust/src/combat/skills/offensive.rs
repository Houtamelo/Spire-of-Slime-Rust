...
impl OffensiveSkill {
	...
	pub fn final_hit_chance(&self, caster: &CombatCharacter, target: &CombatCharacter) -> Option<isize> {
	...
	pub fn final_hit_chance_independent(base_acc: isize, caster: &CombatCharacter, target: &CombatCharacter) -> isize {
		return base_acc + caster.stat(ModifiableStat::ACC) - target.stat(ModifiableStat::DODGE);
	}
	
	pub fn final_crit_chance(&self, caster: &CombatCharacter) -> Option<isize> {
	...
	pub fn final_crit_chance_independent(base_crit: isize, caster: &CombatCharacter) -> isize {
		return base_crit + caster.stat(ModifiableStat::CRIT);
	}
}