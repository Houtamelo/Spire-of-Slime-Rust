...
impl UI_CharacterStats {
	pub fn new(root_node: TRef<Control>,
		...
		macro_rules! get_or_bail {
		    ($path: literal, $node_ty: ty) => {
			    unsafe {
					root_node.get_node_as::<$node_ty>($path)
					         .ok_or_else(|| anyhow!("Failed to {} from {}", $path, root_node.name()))
					         .map(|tref| tref.assume_shared())
				}
		    };
		}
		
		let stamina_fill = get_or_bail!("stamina/fill", Range)?;
		let stamina_label = get_or_bail!("stamina/value", Label)?;
		
		let temptation_fill = get_or_bail!("temptation/fill", Range)?;
		let temptation_label = get_or_bail!("temptation/value", Label)?;
		
		let lust_low_fill = get_or_bail!("lust/low_fill", Range)?;
		let lust_high_fill = get_or_bail!("lust/high_fill", Range)?;
		let lust_label = get_or_bail!("lust/value", Label)?;
		
		let dmg_label = get_or_bail!("grid-stats/damage/value", Label)?;
		let power_label = get_or_bail!("grid-stats/power/value", Label)?;
		let acc_label = get_or_bail!("grid-stats/accuracy/value", Label)?;
		let crit_rate_label = get_or_bail!("grid-stats/crit-rate/value", Label)?;
		let speed_label = get_or_bail!("grid-stats/speed/value", Label)?;
		let dodge_label = get_or_bail!("grid-stats/dodge/value", Label)?;
		
		let toughness_label = get_or_bail!("grid-stats/toughness/value", Label)?;
		let composure_label = get_or_bail!("grid-stats/composure/value", Label)?;
		let stun_def_label = get_or_bail!("grid-stats/stun-mitigation/value", Label)?;
		
		let debuff_res_label = get_or_bail!("grid-stats/debuff-resistance/value", Label)?;
		let debuff_rate_label = get_or_bail!("grid-stats/debuff-rate/value", Label)?;
		
		let move_res_label = get_or_bail!("grid-stats/move-resistance/value", Label)?;
		let move_rate_label = get_or_bail!("grid-stats/move-rate/value", Label)?;
		
		let poison_res_label = get_or_bail!("grid-stats/poison-resistance/value", Label)?;
		let poison_rate_label = get_or_bail!("grid-stats/poison-rate/value", Label)?;
