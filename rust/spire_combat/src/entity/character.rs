use crate::effects::*;
use crate::entity::data::girls::{
	ethel::perks::*,
	nema::perks::*,
};
use crate::entity::skill_intention::SkillIntention;
...
pub struct CombatCharacter {
	...
	pub skill_use_counters: HashMap<SkillVariant, u16>,
}
...
impl CombatCharacter {
	...
	pub fn can_be_grappled(&self) -> bool {
		return if let CharacterDataVariant::Girl(_) = &self.data {
	...
	pub fn into_grappled_unchecked(self) -> GrappledGirlEnum {
		...
		let CharacterDataVariant::Girl(girl_data) = self.data else { panic!("impossible"); };
	...
	pub fn do_on_zero_stamina(mut self, killer_option: Option<&mut CombatCharacter>,
		...
		// Perk::Ethel_LingeringToxins
		{
			...
			for effect in self.persistent_effects.iter() {
				let PersistentEffect::Poison {
					duration_ms, accumulated_ms, interval_ms,
					poison_per_interval: dmg_per_interval, additives, caster_guid
				} = effect
					else { continue; };
		...
		match self.on_zero_stamina {
			OnZeroStamina::Corpse => {
				let corpse = Entity::Corpse(Corpse {
					...
					data: EntityDataVariant::Character(self.data),
				});
	...
	pub fn increment_skill_counter(&mut self, skill_name: SkillVariant) {
	...
	pub fn skill_counter_bellow_limit(&self, skill_name: SkillVariant, limit: u16) -> bool {
	...
	pub fn iter_perks_mut(&mut self) -> impl Iterator<Item = &mut Perk> {
		self.perks
		    .iter_mut()
		    .chain(
			    self.persistent_effects
