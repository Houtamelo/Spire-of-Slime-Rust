...
use crate::effects::*;
use crate::entity::data::girls::ethel::perks::EthelPerk;
use crate::entity::data::girls::ethel::skills::EthelSkill;
use crate::entity::data::girls::nema::perks::NemaPerk;
use crate::perk::{get_perk_mut, has_perk};
...
pub fn start(mut caster: CombatCharacter, target: CombatCharacter, others: &mut HashMap<Uuid, Entity>,
	...
	if matches!(name, SkillVariant::Ethel(EthelSkill::Clash | EthelSkill::Pierce | EthelSkill::Sever))
	...
	} else if matches!(name, SkillVariant::Ethel(EthelSkill::Challenge))
...
fn process_self_effects_and_costs(caster: &mut CombatCharacter, others: &mut HashMap<Uuid, Entity>,
	...
	caster.increment_skill_counter(skill.variant());
...
fn resolve_target(mut caster: CombatCharacter, mut target: CombatCharacter,
	...
	let damage = {
		...
		if matches!(skill.variant(), SkillVariant::Ethel(EthelSkill::Clash | EthelSkill::Sever))
	...
	return match &target.state {
		...
		CharacterState::Grappling(..) => {
			let CharacterState::Grappling(grappling_state) = mem::replace(&mut target.state, CharacterState::Idle)
				else { unreachable!(); };
	...
	fn check_riposte(mut caster: CombatCharacter, target_as_riposter: &mut CombatCharacter,
		...
		let Some((skill_power, acc_mode, crit_mode)) =
			target_as_riposter
				.persistent_effects
				.iter()
				.find_map(|effect| {
					if let PersistentEffect::Riposte { skill_power, acc_mode, crit_mode, .. } = effect {
						Some((skill_power.get(), *acc_mode, *crit_mode))
					} else {
						None
					}
				})
			...
			match crit_mode {
				...
				CRITMode::CanCrit { chance } => {
					let final_crit_chance =
						OffensiveSkill::final_crit_chance_independent(chance, target_as_riposter);
		...
		if is_crit {
			target_as_riposter
				.iter_perks_mut()
				.for_each(|perk| {
					if let Perk::Ethel(EthelPerk::Crit_Vicious { stacks }) = perk {
						*stacks -= 2;
					}
				})
		}
		...
		let dmg_range = OffensiveSkill::calc_dmg_independent(
			skill_power.into(), ToughnessReduction::new(0), target_as_riposter, &caster, is_crit);

		let damage = rng.gen_range(dmg_range);
		if damage <= 0 {
			return Some(caster);
		}
