...
use crate::combat::skills::{DefensiveSkill, SkillType};
...
impl CombatState {
	...
	fn tick(&mut self, delta_time_ms: i64) {
		...
		while let Some(guid) = guids_to_tick.pop() {
			if let Some(position) = left_entities.iter().position(|entity| 
					match entity {
			...
			if let Some(position) = right_entities.iter().position(|entity| 
					match entity {
						Entity::Character(character) => { character.guid == guid }
						_ => { false } 
					}) 
			...
			eprintln!("Warning: Trying to tick character with guid {guid:?}, but it was not found in the left or right entities!");
		}
		...
		fn tick_character(ticked_character: &mut CombatCharacter, allies: &mut Vec<Entity>, enemies: &mut Vec<Entity>, delta_time_ms: i64) {
			...
			match state {
				...
				CharacterState::Charging { skill_intention } => { 
					let mut skill_intention_clone = skill_intention.clone(); // clone is needed because we need to pass ticked_character as immutable to calculate it's SPD
					
					let spd_delta_time_ms = CharacterState::spd_charge_ms(delta_time_ms, ticked_character.stat(SPD));
					skill_intention_clone.charge_ticks.remaining_ms -= spd_delta_time_ms;
					if skill_intention_clone.charge_ticks.remaining_ms <= 0 {
						//todo! Cast skill
						ticked_character.state = match skill_intention_clone.recovery_after_complete {
							Some(ticks) => { CharacterState::Recovering { ticks } }
							None => { CharacterState::Idle }
						}
					}
					else {
						ticked_character.state = CharacterState::Charging { skill_intention: skill_intention_clone };
					}
				},
		...
		fn tick_grappled_girl(state: &mut CharacterState, ticked_position: &Position, delta_time_ms: i64, enemies: &mut Vec<Entity>) {
			let CharacterState::Grappling {victim, lust_per_sec, temptation_per_sec, duration_ms, accumulated_ms} = state else {
				panic!()
			};
		...
		fn cast_skill_self(caster: &mut CombatCharacter, skill: DefensiveSkill, recover_ms: Option<i64>, allies: &mut Vec<Entity>, enemies: &mut Vec<Entity>, seed: &mut StdRng) {
			let crit_chance: Option<isize> = skill.calc_crit_chance(caster);

			let crit_self = match crit_chance { 
				Some(chance) if seed.base100_chance(chance) => { true },
				_ => false 
			};
			
			for self_applier in skill.effects_self {
				self_applier.apply(caster, seed, crit_self, allies, enemies);
			}
		}
	}
